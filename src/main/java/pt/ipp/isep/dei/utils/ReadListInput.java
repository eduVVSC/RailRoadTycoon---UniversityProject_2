...
public class ReadListInput {
    ...
    public static void requestUserInputList(InputReceiver receiver) {
        Platform.runLater(() -> {
            try {
                FXMLLoader loader = new FXMLLoader(ReadListInputController.class.getResource("/fxml/ReadListInput.fxml"));

                // Inject controller with callback
                ReadListInputController controller = new ReadListInputController(receiver);
                ...
                controller.initialize(App.getMessage(), App.getList());
